...
export class ExportService {
  ...
  private tripReportFileName: string = "trip_spendings.xlsx";
  ...
  generateTripSpendingReport(tripId: Number): void {
    this.http.get(EXPORT_API + "tripSpending/" + tripId,
      {
        headers: new HttpHeaders().set('Authorization', 'Bearer ' + this.storageService.getUser().token),
        responseType: 'blob'
      }
      ...
      .subscribe(
        blob => {
          saveAs(blob, this.tripReportFileName);
        },
        error => {
          this.toasterService.error("Error when generating trip spending report: " + error);
        }
      )
  }
}
